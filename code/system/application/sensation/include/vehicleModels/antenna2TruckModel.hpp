...
public:
    ...
    S f(const S& x, const C& u) const
    {
        ...
        double pitch = 0; //TODO get pitch from IMU
        double height = 4; //TODO get a better value of height

        // calculate the X - components
        x_p.x() = x.x() + std::cos(x.theta())*std::tan(pitch)*height + delta_t * x.x_dot();
        x_p.x_dot() = u.v()*std::cos(x.theta()) - lateral_acceleration * (std::sin(x.theta())) * delta_t;

        // calculate the Y - components
        x_p.y() = x.y() + std::sin(x.theta())*std::tan(pitch)*height + delta_t * x.y_dot();
